...
import com.cj.mobile.myarapplication.util.ImageUtils;
...
/**
 * 关于获取图像监听器
 * 类，它接收预览帧并将图像转换为位图，以便使用dlib-lib进行处理。
 */
public class OnGetImageListener implements OnImageAvailableListener {
     ...
     * @param handler
     */
    public void initialize(final Context context, final Handler handler) {
        ...
        mInferenceHandler.post(
                () -> {
                    if (!new File(Constants.getFaceShapeModelPath()).exists()) {
                        throw new RuntimeException("cannot find " + Constants.MODEL_PATH);
                    }
